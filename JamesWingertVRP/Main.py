...
class Main:
    # Main UI when the program starts up
    print('Package Tracking System')
    print('Route was completed in', "{0:.2f}".format(total_distance(), 2), 'miles.')
    start = input("Type 'package' to look up by package number,or  "
                  "type 'time' to see all package status' at a given time: ")
    # Space-time complexity is O(N)
    while start != 'exit':

        # If user enters 'time' it shows all packages at a certain time frame and the status of packages
        if start == 'time':
            try:
                ...
                print('Invalid entry!')
                exit()
        # If user enters 'package' user is able to check specific package at a specific time
        elif start == 'package':
            try:
