...
from ReadCSV import check_truck_one
from ReadCSV import check_truck_two
from ReadCSV import check_truck_one_trip_two
...
from Distances import truck_one_optimized_index
from Distances import truck_one_optimized_list
from Distances import truck_two_optimized_index
from Distances import truck_two_optimized_list
from Distances import truck_three_optimized_index
from Distances import truck_three_optimized_list
...
for value in check_truck_one():
    check_truck_one()[i][9] = first_time
    first_delivery.append(check_truck_one()[i])
    i += 1
...
for index in range(len(truck_one_optimized_index())):
    try:
        # calculate the total distance of the truck
        first_truck_total_distance = check_distance(int(truck_one_optimized_index()[index]), int(truck_one_optimized_index()[index + 1]), first_truck_total_distance)
        # calculate the distance of each package along the route
        deliver_package = check_time_first_truck(check_current_distance(int(truck_one_optimized_index()[index]), int(truck_one_optimized_index()[index + 1])))
        truck_one_optimized_list()[first_truck_package_id][10] = (str(deliver_package))
        get_hash_map().update(int(truck_one_optimized_list()[first_truck_package_id][0]), first_delivery)
...
for value in check_truck_two():
    check_truck_two()[i][9] = second_time
    second_delivery.append(check_truck_two()[i])
    i += 1
...
for index in range(len(truck_two_optimized_index())):
    try:
        # calculate the total distance of the truck
        second_truck_total_distance = check_distance(int(truck_two_optimized_index()[index]), int(truck_two_optimized_index()[index + 1]), second_truck_total_distance)
        # calculate the distance of each package along the route
        deliver_package = check_time_second_truck(check_current_distance(int(truck_two_optimized_index()[index]), int(truck_two_optimized_index()[index + 1])))
        truck_two_optimized_list()[second_truck_package_id][10] = (str(deliver_package))
        get_hash_map().update(int(truck_two_optimized_list()[second_truck_package_id][0]), second_delivery)
...
for index in range(len(truck_three_optimized_index())):
    try:
        # calculate the total distance of the truck
        third_truck_total_distance = check_distance(int(truck_three_optimized_index()[index]), int(truck_three_optimized_index()[index + 1]), third_truck_total_distance)
        # calculate the distance of each package along the route
        deliver_package = check_time_third_truck(check_current_distance(int(truck_three_optimized_index()[index]), int(truck_three_optimized_index()[index + 1])))
        truck_three_optimized_list()[third_truck_package_id][10] = (str(deliver_package))
        get_hash_map().update(int(truck_three_optimized_list()[third_truck_package_id][0]), third_delivery)
